...
class PhysXWheelContactModifyCallback : public PxContactModifyCallback
{
public:

	PhysXWheelContactModifyCallback()
	...
	void onContactModify(PxContactModifyPair* const pairs, PxU32 count)
	{
		for (PxU32 i = 0; i < count; i++)
		{
			...
			for (PxU32 j = 0; j < 2; j++)
			{
				...
				if (actor->userData && (static_cast<PActorUserData*>(actor->userData))->vehicle)
				{
					const PxVehicleWheels* vehicle = (static_cast<PActorUserData*>(actor->userData))->vehicle;
					PX_ASSERT(vehicle->getRigidDynamicActor() == actors[j]);

					const PxShape* shape = shapes[j];
					if (shape->userData && (static_cast<PShapeUserData*>(shape->userData))->isWheel)
					{
						const PxU32 wheelId = (static_cast<PShapeUserData*>(shape->userData))->wheelId;
...
PhysXWheelContactModifyCallback gWheelContactModifyCallback;